...
const Navbar = () => {
    ...
    return (
        <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'backdrop-blur-md' : ''
            }`}>
            ...
            <div className="mx-auto px-6 sm:px-12 lg:max-w-[70rem] xl:max-w-[76rem] 2xl:max-w-[92rem]">
                <div className="relative">
                            ...
                            <div className="md:hidden flex">
                                ...
                                >
                                    <div className="flex flex-col space-y-1">
                                        <div className={`w-6 h-0.5 bg-white transition-all duration-300 group-hover:bg-cyber-magenta ${isMobileMenuOpen ? 'rotate-45 translate-y-1.5' : ''
                                            }`}></div>
                                        <div className={`w-6 h-0.5 bg-white transition-all duration-300 group-hover:bg-cyber-magenta ${isMobileMenuOpen ? 'opacity-0' : ''
                                            }`}></div>
                                        <div className={`w-6 h-0.5 bg-white transition-all duration-300 group-hover:bg-cyber-magenta ${isMobileMenuOpen ? '-rotate-45 -translate-y-1.5' : ''
                                            }`}></div>
                                    </div>
                    ...
                    {/* Mobile Menu */}
                    <div className={`md:hidden absolute top-full left-4 right-4 mt-2 transition-all duration-300 z-40 ${isMobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'
                        }`}>
                        <div className="bg-gradient-to-r from-black/60 via-cyber-surface/80 to-black/60 backdrop-blur-xl 
                                      border border-cyber-cyan/50 rounded-lg overflow-hidden">
                            {/* Corner Indicators for Mobile Menu */}
                            <div className="absolute -top-1 -left-1 w-2 h-2 border-t-2 border-l-2 border-cyber-cyan"></div>
                            <div className="absolute -top-1 -right-1 w-2 h-2 border-t-2 border-r-2 border-cyber-cyan"></div>
                            <div className="absolute -bottom-1 -left-1 w-2 h-2 border-b-2 border-l-2 border-cyber-cyan"></div>
                            <div className="absolute -bottom-1 -right-1 w-2 h-2 border-b-2 border-r-2 border-cyber-cyan"></div>
                                ...
                                {navigationLinks.map((link) => (
                                    <li key={link.path}>
                                        <Link
                                            href={link.path}
                                            onClick={(e) => handleSmoothScroll(e, link.path)}
                                            className="group block px-4 py-3 no-underline outline-none border-b border-cyber-cyan/10 last:border-b-0"
                                        >
                                            <div className="flex items-center justify-between">
                                                <span className="text-white/80 font-mono tracking-wide
                                                               transition-all duration-300 
                                                               group-hover:text-cyber-cyan
                                                               group-hover:drop-shadow-[0_0_8px_#00ffff]">
                                                    {link.name.toUpperCase()}
                                                </span>
                                                <div className="w-2 h-2 border border-cyber-cyan/50 rotate-45 
                                                             transition-all duration-300 group-hover:border-cyber-cyan"></div>
                                            </div>
                                            {/* Mobile Scan Line */}
                                            <div className="h-px w-0 bg-gradient-to-r from-cyber-cyan to-cyber-magenta
                                                          group-hover:w-full transition-all duration-500 mt-2"></div>
                                        </Link>
                                    </li>
                                ))}
                            </ul>
